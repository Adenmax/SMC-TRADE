...
def get_overall_bias(df_daily, df_4h, df_1h, df_15min):
    ...
    # Extra strictness: if 4H conflicts with majority, require 4/4
    h4_bias = biases.get("4h", "ranging")
    if direction == "bullish" and h4_bias == "bearish" and bull < 4:
        direction = "ranging"  # 4H against us with only 3/4 = skip
    if direction == "bearish" and h4_bias == "bullish" and bear < 4:
        direction = "ranging"  # 4H against us with only 3/4 = skip
...
def check_imbalance(df, direction="bullish", ob=None, search_range=10,
    ...
    FVG must be at least min_gap_pct wide (filters out micro/fake gaps).
    Default 0.02% = ~0.8 pips on Gold at 4000, ~0.04 pips on GBP/JPY.
    """
    ...
    for i in range(search_start + 2, search_end):
        if direction == "bullish" and l[i] > h[i - 2]:
            ...
            gap = top - bot
            mid = (top + bot) / 2
            # Minimum gap filter — reject micro gaps
            if gap < mid * min_gap_pct:
                continue
            return True, {"top": top, "bot": bot, "bar": i}
...
def confirm_choch(df, direction="bullish", swing_n=2):
    ...
    if direction == "bullish":
        ...
        if body_high > last_lower_high and c[-2] <= last_lower_high:
            return True, last_lower_high, "CHoCH"
    else:
        lows = [l[i] for i in range(len(l)) if pl[i]]
        if not lows:
            return False, None, None
        ...
        if body_low < last_higher_low and c[-2] >= last_higher_low:
            return True, last_higher_low, "CHoCH"
...
def confirm_engulfing(df, direction="bullish"):
    ...
    Body of current candle fully engulfs previous candle body.
    Returns (confirmed, None, conf_type)
    """
...
def get_confirmation(df_5min, df_1min, direction):
    """
    Check confirmations on 5min first, then 1min.
    Types: CHoCH (strongest) then Engulfing.
    Returns (confirmed, level, conf_type, tf_name)
    """
    for tf_name, df in [("5min", df_5min), ("1min", df_1min)]:
        if df is None or len(df) < 5:
            continue
        # 1. CHoCH
        ok, level, ctype = confirm_choch(df, direction)
        if ok:
            return True, level, ctype, tf_name
        # 2. Engulfing
        ok, level, ctype = confirm_engulfing(df, direction)
        if ok:
            return True, level, ctype, tf_name
...
def run_full_analysis(df_daily, df_4h, df_1h, df_15min,
    ...
    if bos_found:
        ...
        if ob:
            ...
            # OB must be fresh: price approaching from outside, not already inside
            # If OB was formed recently (last 5 bars) and price is inside = valid tap
            # If OB is old and price has been inside for multiple bars = already mitigated
            ob_bar_idx = ob.get("bar", 0)
            bars_since_ob = len(df_15min) - 2 - ob_bar_idx
            ob_tapped = price_in_ob(current_price, ob)

            # Reject if price has been hovering inside OB for too long (mitigated)
            if ob_tapped and bars_since_ob > 0:
                # Check if price was already inside OB in previous bar
                prev_price = float(df_15min["close"].iloc[-3]) if len(df_15min) >= 3 else current_price
                already_inside = price_in_ob(prev_price, ob)
                # If price was inside OB last bar too = zone already being mitigated
                if already_inside and bars_since_ob > 3:
                    ob_tapped = False  # treat as mitigated, not a fresh tap

            result["ob_tapped"] = ob_tapped

            # ── Confirmation: 5min CHoCH ──────────────────────────────────────
            if ob_tapped and df_5min is not None:
                confirmed, conf_level, conf_type, conf_tf = get_confirmation(
                ...
                if confirmed and all_confluences and conf_type is not None:
