...
def get_bias(df, swing_n=3):
    """
    Detect bias using two methods:
    1. Swing structure (HH/HL or LH/LL)
    2. Price vs recent midpoint (fallback)
    Uses most recent swing points — not historical ones.
    """
    ...
    # Method 2: price vs 50-bar midpoint (trend filter)
    ...
    # Strong trend: current price clearly above/below midpoint
...
def get_prev_day_levels(df_daily):
    """Previous day high and low — key reference levels."""
...
def get_friday_levels(df_daily):
    """Most recent Friday high and low — weekly reference level."""
    if df_daily is None or "datetime" not in df_daily.columns:
        return None, None
    df = df_daily.copy()
    df["weekday"] = df["datetime"].dt.weekday  # 4 = Friday
    fridays = df[df["weekday"] == 4]
    if len(fridays) == 0:
        return None, None
    last_fri = fridays.iloc[-1]
    return float(last_fri["high"]), float(last_fri["low"])


def get_daily_swing_levels(df_daily, swing_n=3):
    """Most recent daily swing high and swing low."""
    if df_daily is None or len(df_daily) < swing_n * 2 + 2:
        return None, None
    h = df_daily["high"].values
    l = df_daily["low"].values
    ph, pl = _swing_points(h, l, swing_n)
    ...
    last_sh = float(swing_highs[-1]) if swing_highs else None
    last_sl = float(swing_lows[-1])  if swing_lows  else None
    return last_sh, last_sl


def get_daily_fvgs(df_daily, direction="bullish"):
    """FVGs on the daily chart — major institutional zones."""
    return get_fvgs(df_daily, direction, max_age=20)

def build_zones(df_daily, df_1h, df_15min, direction, current_price=0):
    """
    Build all candidate zones for the given direction.
    Priority: most recent untapped zone first.
    Returns list sorted by proximity to current price.
    """
    zones = []
    buf = 0.002  # 0.2% buffer for level-based zones
    ...
    # ── Daily FVGs (untapped) ────────────────────────────────────────────────
    fvgs_d = get_daily_fvgs(df_daily, direction)
    for z in fvgs_d:
        if not z["tapped"]:
            z["tf"] = "Daily"
            z["confluence"] = ["Daily FVG"]
            zones.append(z)
    ...
    if direction == "bullish" and prev_l:
        z = {"top": prev_l * (1 + buf), "bot": prev_l * (1 - buf),
             "bar": 999, "tapped": current_price <= prev_l * (1 + buf),
             "direction": "bullish", "type": "prev_day",
             "tf": "Daily", "confluence": ["Prev day low"]}
        if not z["tapped"]:
            zones.append(z)
    if direction == "bearish" and prev_h:
        z = {"top": prev_h * (1 + buf), "bot": prev_h * (1 - buf),
             "bar": 999, "tapped": current_price >= prev_h * (1 - buf),
             "direction": "bearish", "type": "prev_day",
             "tf": "Daily", "confluence": ["Prev day high"]}
        if not z["tapped"]:
            zones.append(z)

    # ── Friday high/low ───────────────────────────────────────────────────────
    fri_h, fri_l = get_friday_levels(df_daily)
    if direction == "bullish" and fri_l:
        z = {"top": fri_l * (1 + buf), "bot": fri_l * (1 - buf),
             "bar": 998, "tapped": current_price <= fri_l * (1 + buf),
             "direction": "bullish", "type": "friday",
             "tf": "Weekly", "confluence": ["Friday low"]}
        if not z["tapped"]:
            zones.append(z)
    if direction == "bearish" and fri_h:
        z = {"top": fri_h * (1 + buf), "bot": fri_h * (1 - buf),
             "bar": 998, "tapped": current_price >= fri_h * (1 - buf),
             "direction": "bearish", "type": "friday",
             "tf": "Weekly", "confluence": ["Friday high"]}
        if not z["tapped"]:
            zones.append(z)

    # ── Daily swing high/low ──────────────────────────────────────────────────
    d_sh, d_sl = get_daily_swing_levels(df_daily)
    if direction == "bullish" and d_sl:
        z = {"top": d_sl * (1 + buf), "bot": d_sl * (1 - buf),
             "bar": 997, "tapped": current_price <= d_sl * (1 + buf),
             "direction": "bullish", "type": "daily_swing",
             "tf": "Daily", "confluence": ["Daily swing low"]}
        if not z["tapped"]:
            zones.append(z)
    if direction == "bearish" and d_sh:
        z = {"top": d_sh * (1 + buf), "bot": d_sh * (1 - buf),
             "bar": 997, "tapped": current_price >= d_sh * (1 - buf),
             "direction": "bearish", "type": "daily_swing",
             "tf": "Daily", "confluence": ["Daily swing high"]}
        if not z["tapped"]:
            zones.append(z)
...
def run_full_analysis(df_daily, df_4h, df_1h, df_15min,
    ...
    # Build zones
    zones = build_zones(df_daily, df_1h, df_15min, direction, current_price)
    result["zones"] = zones
